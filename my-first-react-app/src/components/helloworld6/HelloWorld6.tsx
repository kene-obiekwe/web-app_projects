...
const HelloWorld6: React.FC<Props> = ({ name = "Ife" }) => {
    ...
    const setUserIdToFetch = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.value !== '') { // Ensure user fetch does not happen when id is wiped
    ...
    const fetchData = async () => {
        try {
            ...
            if (response.status < 200 || response.status >= 300) {
                setUserFetched({ user: null, error: `No user to display; Response status = ${response.status}` });
                return;
            }
